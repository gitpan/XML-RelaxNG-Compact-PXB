...
=item elements => [parameter => $parameter] - defines single B<parameter>  sub-element

=item elements => [parameter => [$parameter]] - defines list of B<parameter> sub-elements

=item elements => [parameters => [$parameters, $select_parameters]] - defines choice between 
                   two single C<parameters> sub-elements of different type

=item elements => [datum => [[$pinger_datum],[$result_datum]] ] - defines choice between two lists of B<datum> 
                  sub-elements of different type 
