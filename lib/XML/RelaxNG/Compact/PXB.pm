...
sub new {
    ...
    $self->footer->see_also(' Automatically generated by L<XML::RelaxNG::Compact::PXB> ');
    return $self;
}
...
sub buildAM {
    my ($self, $arr_names) = @_;
    croak " Only array ref parameter accepted" . Dumper $arr_names  unless ref($arr_names) eq 'ARRAY';
...
####printing  namespace specific helper classes
    ...
    foreach my $el (@elementnodes) {
        foreach my $ns (keys %{$self->{_known_class}->{$el->[0]}}) {
              $self->sayIt("use " . $self->{_known_class}->{$el->[0]}{$ns} . ";")  if $self->{_known_class}->{$el->[0]}{$ns};
        }
    }
...
###
    ...
    foreach my $els (@elementnodes) {
        croak(" Malformed elements declaration: name=$name  and this thingy: els=$els must be an ARRAY ref ") unless ref($els) eq 'ARRAY';
    ...
    foreach my \$textnode (qw/$texts_names/) {
        if(\$self->{\$textnode}) {
           ...
           \$domtext?\$$name->appendChild(\$domtext):\$self->get_LOGGER->logdie("Failed to append new text element \$textnode  to  $name");
        }
    }
        !);
    }
...
### print accessors/mutators for all element nodes and attributes ( careless about the type of the element )
    ...
    $self->buildAM(['LOGGER', 'nsmap', 'idmap', 'text',   @attributes, map { $_->[0] } @elementnodes, @textnodes]);
...
sub add\u${subname} {
    ...
    \$self->get_$subname && ref(\$self->get_$subname) eq 'ARRAY'?push \@{\$self->get_$subname}, \$new:\$self->set_$subname([\$new]);
