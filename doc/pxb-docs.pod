...
=item  elements =>  [parameter => [$parameter]]  - defines list of  C<‘‘parameter’> sub-elements

=item  elements => [parameter => $parameter]  - defines single C<‘parameter>’ sub-element

=item  elements =>  [parameters  => [$parameters,$select_parameters]]  - defines choice between  two single ‘C<parameters>’ sub-elements of different type

=item  elements =>  [datum  => [[$pinger_datum], [$result_datum]] ]  - defines choice between  two lists of  C<‘‘datum>’ sub-elements of different type

=back

In C<elements-definition> the third member  is an optional conditional statement which represents validation rule. 
For example C<‘unless:value>’ conditional statement  will be translated  into the  perl’  conditional statement  
...
of the class fields with support for elements identified by C<id>. It supports special field, 
named idmap  for that purpose. It “knows” how to serialize object of the class into the DOM or XML 
text string and how to map contents of the object on some SQL schema. By issuing C<registerNamespaces> call to 
...
Our case is based on building interoperable SOAP document/literal based webservice for C<perfSONAR-PS> project.
...
=item 1. RelaxNG Compact - L<http://relaxng.org/compact-tutorial-20030326.html>

=item 2 xmlbeans , L<http://xmlbeans.apache.org>

=item 3 METRO, L<http://java.sun.com/webservices/index.jsp>

=item 4 CASTOR, L<http://www.castor.org/>

=item 5 JAXB,  L<http://java.sun.com/developer/technicalArticles/WebServices/jaxb/>
 
=item 6 Perl Best Practices, by Damian Conway

=item 7 EBNF - L<http://en.wikipedia.org/wiki/Extended_Backus%E2%80%93Naur_form>

=item 8 perfSONAR-PS - L<https://wiki.internet2.edu/confluence/display/PSPS>
